...
int main(){
    ...
    printf("entier geant = %lld\n", *my_geant_addr);
    printf("adresse entier geant = %p\n", my_geant_addr);

    printf("rentrez un chiffre a virgule ");
    float virgule = 0;
    scanf("%f",&virgule);
    float *my_virgule_addr = &virgule;
    printf("chiffre a virgule = %f\n", *my_virgule_addr);
    printf("chiffre a virgule = %p\n", my_virgule_addr);

    printf("rentrez grand chiffre a virgule ");
    double grand_v= 0;
    scanf("%lf",&grand_v);
    double *my_grand_v_addr = &grand_v;
    printf("grand chiffre a virgule = %lf\n", *my_grand_v_addr);
    printf("grand chiffre a virgule = %p\n", my_grand_v_addr);

    printf("rentrez géant a virgule ");
    long double geant_v = 0;
    scanf("%Lf",&geant_v);
    long double *my_geant_v_addr = &geant_v;
    printf("geant a virgule = %Lf\n", *my_geant_v_addr);
    printf("adresse geant a virgule = %p\n", my_geant_v_addr);
}